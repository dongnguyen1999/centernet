...
def fix_outside_box(box, width, height):
    x1, y1, x2, y2 = box
    nx1, ny1, nx2, ny2 = box

    if x1 < 0: nx1 = 0
    if x1 > width-1: nx1 = width-1
    if x2 < 0: nx2 = 0
    if x2 > width-1: nx2 = width-1

    if y1 < 0: ny1 = 0
    if y1 > height-1: ny1 = height-1
    if y2 < 0: ny2 = 0
    if y2 > height-1: ny2 = height-1

    dx, dy = (x2 - x1), (y2 - y1)
    area = dx*dy
    if dx < 0 or dy < 0 or area == 0: return None

    dx, dy = (nx2 - nx1), (ny2 - ny1)
    area2 = dx*dy
    if dx < 0 or dy < 0 or area2 == 0: return None

    cx, cy = (nx1 + (nx2-nx1)*0.5), (ny1 + (ny2-ny1)*0.5)
    if cx < 0 or cx > width-1 or cy < 0 or cy > height-1: return None

    if (area2/area) > 0.3: return [nx1, ny1, nx2, ny2]
