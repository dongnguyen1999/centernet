...
class FrameDiffEstimator:
    ...
    def refresh(self):
        self.long_subtractor = cv2.createBackgroundSubtractorMOG2(history=self.long_term_history, varThreshold=self.threshold, detectShadows=False)
        self.short_subtractor = cv2.createBackgroundSubtractorMOG2(history=self.short_term_history, varThreshold=self.threshold, detectShadows=False)
    ...
    def apply(self, frame, mask=None):
        ...
        return (np.sum(diff_mask > 0) * 100) / np.sum(foreground_mask > 0)
